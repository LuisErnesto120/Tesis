...
Sin embargo, es posible inferir información sobre la posible temperatura a la que ocurre la cristalización al comparar los datos obtenidos con la temperatura de los cambios que se espera ocurran en la muestra. En este caso, se espera que haya una combustión de la fase orgánica y la evaporación del agua contenida en la muestra entre los 100 y 200\gradoC{}, mientras que la fase metálica es estable hasta temperaturas mucho más altas que las máximas alcanzadas por el equipo ($\approx1000$\gradoC{}). Por esta razón, se infiere que, a una temperatura mayor a 200\gradoC{} es posible observar un proceso asociado a la cristalización.
...
\begin{figure}[H]
    ...
    \caption{Diagrama de un arreglo experimental típico utilizado para la obtención de espectros de difracción de rayos X. Adaptado de \cite{Jung2023}}
    \label{fig:diagDRX}
\end{figure}
...
Si se representa cada átomo perteneciente a un cristal como un punto, podemos expresar un cristal en 3 dimensiones como el conjunto de puntos que cumplen:
\begin{equation}
...
\begin{equation}
    ...
    \label{eq:leydebragg}
\end{equation}
Esta ecuación se denomina ley de Bragg, la cual proporciona información sobre la estructura cristalina de una estructura dada a través de sus ángulos de difracción \cite{Cullity2014}.

Para obtener los ángulos de difracción de cada ortoferrita se utilizó el programa \textit{VESTA} \cite{VESTA}, en conjunto con datos sobre las estructuras cristalinas reportados en \textit{Crystallography Open Database} \cite{COD}.
\subsection{Refinamiento Rietveld}
El método de Rietveld es una técnica de refinamiento que permite estudiar los patrones de difracción generados por rayos X de manera cuantitativa. Esta técnica permite encontrar información sobre la composición de la muestra, las fases cristalinas existentes en ésta y la posible presencia de contaminantes o fases cristalinas secundarias presentes en la muestra a analizar.
...
\begin{figure}[H]
    ...
    \caption{Modelado de picos de difracción utilizando \textit{MAUD}. La gráfica superior muestra las mediciones de intensidad respecto al ángulo en negro, con la curva ajustada en rojo, debajo de éste se encuentran las estructuras cristalinas y los picos que se observan de éstas en el difractograma y en la gráfica inferior se tiene el residuo del ajuste.}
...
Funcionan haciendo uso de una fuente de electrones, los cuales son acelerados con un potencial de 30 kV y posteriormente son focalizados haciendo uso de lentes magnéticos. Se utilizan comúnmente tres de éstos, dos condensadores y un objetivo, siendo éste un determinante en la calidad de la imagen. Como resultado se produce un haz de electrones con un diámetro de entre 1 y 10 nm.
...
\subsection{Interacciones Electrón-Sólido} \label{sec:electronsolido}
...
Las imágenes obtenidas a través de SEM fueron analizadas utilizando el software ImageJ \cite{ImageJ}. Para llevar a cabo el análisis estadístico del tamaño de partícula se utilizaron las imágenes con una amplificación de $\times500$ debido a que este valor permite observar de manera completa las partículas grandes mientras siguen siendo visibles las más pequeñas, como se puede ver en la figura \ref{fig:imgjcomp} a).
...
\begin{figure}[H]
    ...
    \caption{Procesado de imágenes con ImageJ. En a) se muestra la imagen original con una amplificación de x500, b) muestra la imagen después de aplicar la función \textit{Threshold}, c) la misma imagen después de aplicar la función \textit{Analyze Particles}.}
    \label{fig:imgjcomp}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Histogramas de los datos obtenidos con ImageJ mediante a) medición manual del diámetro de las partículas, b) medición del diámetro de las partículas mediante la función preestablecida.}
...
Después, el haz resultante se incide primero en una muestra en blanco para medir el fondo y restar éste de los datos que se obtengan, para luego incidir el haz sobre la muestra de interés y registrar ya sea la absorbancia o reflectancia a través de detectores, los cuales funcionan haciendo uso de semiconductores o una capa fotoeléctrica, debido a que ambos generan una corriente proporcional a la intensidad de la luz incidente \cite{Tom2023}.

En el caso de las mediciones presentadas en esta tesis, se utilizó un espectrómetro modelo \textit{Macylab UV-1800CPC} en modo de absorción en precisión normal.

Se colocó un portamuestras recubierto con \ce{BaSO4} como fondo, con el cual se calibró el fondo utilizado para las mediciones. Posteriormente se realizó la medición con aproximadamente 0.05 g de la muestra, utilizando luz de 200 a 800 nm, con un paso de 0.2 nm entre cada punto.
...
Se midió la magnetización contra la temperatura ($M$ vs. $T$), a partir de ésta se obtuvo la susceptibilidad contra la temperatura ($\chi$ vs. $T$) utilizando el mismo equipo que en las mediciones $M$ vs. $H$. Las curvas $FC$ (\textit{Field Cooling}) se obtuvieron con un campo externo de $50$ mT al variarse la temperatura, mientras que para las mediciones  ZCF (\textit{Zero Field Cooling}) no se aplicó un campo externo al variar la temperatura. El rango de temperatura en el que se realizan las mediciones FC-ZFC es $5$ -$300$ K.
